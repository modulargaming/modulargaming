	...
	$.dform.subscribe({
		...
		"caption" : function (options, type) {
			...
			if (type == "fieldset") {
				...
				} else */if(type != 'bootstrap_tabs'){
					this.parent().parent().prepend(label.addClass('control-label'));
		...
		"type" : function (options, type) {
			...
			if(type != 'fieldset' && type != 'form' && type != 'label' && type != 'options' && type!= 'option' && type != 'bootstrap_tabs' && type != '' && type !='div' ) {
				this.wrap('<div class="control-group"></div>');
				this.parent().find(':input').wrap('<div class="controls"></div>');
			}
		},
		/**
		...
		"[post]" : function (options, type) {
			if(type == 'checkbox' && 'value' in options)
			{
				$(this).attr('checked', options.value);
			}
			else if(type == 'select' && 'multiple' in options)
			{
